...
def parse_args(args):
    ...
    parser.add_argument(
        "--worksers", type=int, default=4, help="Number of dataloader workers per GPU."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--validate_dataset_name_lvis", default="objaverse_lvis", type=str
    )
    ...
    # 设置点云特征的维度，默认为 768。
    parser.add_argument(
        "--pc-feat-dim", type=int, default=768, help="Pointcloud feature dimension."
    )

    # 设置点云 Transformer 中的分组大小，默认为 32。
    parser.add_argument(
        "--group-size", type=int, default=32, help="Pointcloud Transformer group size."
    )
    ...
    # 设置教师模型的嵌入维度，默认为 512。
    parser.add_argument(
        "--embed-dim", type=int, default=512, help="teacher embedding dimension."
    )
    ...
    # 指定要加载的 3D zero-shot 模型检查点路径，默认为空字符串。
