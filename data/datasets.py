...
class ModelNet40_openshape(data.Dataset):
    def __init__(self, config):
        ...
        self.seg_cat = config.args.seg_cat

        # 建立一个字典self.classes = {'airplane': 0, 'bathtub': 1, 'bed': 2, 'bench': 3, ...}
        # zip 的作用就是将后面两个一一配对
        self.classes = dict(zip(self.cat, range(len(self.cat))))

        # 字符串拼接 data/test_datasets/modelnet40/test_pc.npy
        # 用来序列化和反序列化对象的机制。加载 .npy 文件时，如果文件中包含了 Python 对象（如列表、字典等）
        # 则需要使用 allow_pickle=True 才能正确加载。
        self.pcs = np.load("%s/test_pc.npy" % self.data_path, allow_pickle=True)
        print(type(self.pcs))

        # 加载data/test_datasets/modelnet40/test_split.json
        # json文件会被加载成一个字典
        # 数据示例：每一个代表一个点云

        # { "name": "test/bench_0185",
        #   "pc_path": "/datasets-slow1/ModelNet40/partial_pc/bench/test/bench_0185_10k.npy",
        #   "category": "bench" }
        self.openshape_split = json.load(
            open("%s/test_split.json" % self.data_path, "r")
        )

        print_log(
            "The size of %s data is %d" % (split, len(self.openshape_split)),
            logger="ModelNet",
        )

        self.shape_names_addr = os.path.join(
            self.data_path, "modelnet40_shape_names.txt"
        )

        self.cate_to_id = (
            {}
        )  # {'airplane':'0',...} self.classes={'airplane':'0',...} value值类型不一样
    ...
    def __len__(self):
        return len(self.my_openshape_split)
        # return len(self.openshape_split)

    def __getitem__(self, index):
        #   pcs[index]里为数据 对应self.openshape_split[index]
        # pc = copy.deepcopy(self.pcs[index])
        pc = copy.deepcopy(self.my_pcs[index])
        xyz = pc["xyz"]
        rgb = pc["rgb"]

        # 规范化 RGB 值到 0 到 1 的范围
        rgb = rgb / 255.0  # 缩放 RGB 值至 0~1之间
        ...
        # 获取类别名称和对应的标签
        label_name = self.my_openshape_split[index]["category"]
        # label_name = self.openshape_split[index]["category"]
...
class ScanObjNN_openshape(data.Dataset):
    def __init__(self, config):
        ...
        self.openshape_data = np.load(
            "%s/xyz_label.npy" % self.data_path, allow_pickle=True
        ).item()

        print_log(
            "The size of Scanobjnn data is %d" % (len(self.openshape_data["xyz"])),
            logger="ScanObjNN",
        )
    ...
    def __init__(self, config):
        ...
        for key in self.data_list_openshape.keys():
            self.file_list.append(
                {"model_id": key, "data_path": self.data_list_openshape[key]}
            )
        print_log(
            f"[DATASET] {len(self.file_list)} instances were loaded", logger="Objaverse"
        )
    ...
    def __getitem__(self, idx):
        ...
        if "text" in self.text_source:
            if "-Objaverse" in data_path:
                if not (
                    self.use_text_filtering and self.gpt4_filtering[name]["flag"] == "N"
                ):
                    try:
...
class Objaverse_lvis_openshape(data.Dataset):
    def __init__(self, config):
        ...
        print_log(
            f"[DATASET] sample out {self.sample_points_num} points", logger="Objaverse"
        )
